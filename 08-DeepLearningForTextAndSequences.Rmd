...
__An example that works well__
...
We want to use `layer_conv_1d`, it takes 3D input, hence _(samples, time, features)_.
...
_Summary: We observe that using only conv layers does not work well, as it is important with the time of the observation is in the right order, for instance a night temperature might look the same as the daily temperature during colder months. This it will not now how to_
...
For some reason the following cannot run. __NEEDS REVISIT__.
