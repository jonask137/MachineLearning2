...
10. Total conversion: Total number of people who sign up to the webshop after seeing the ad
11. Approved conversion/leads: Total number of people who bought the product after seeing the ad
...
# - Cost Per Mille: the cost of one thousand impressions. If our objective is ad exposure to increase brand awareness,this might be an important to measure. CPM = Spent/Impr *1000.
...
```{r}
# Dealing with missing, currupt and invalid data
...
The following is the same, but presented just with correlations.
...
Feature selection approaches include: 
 ...
 - c3. advanced feature selection methods based on other data-mining techniques, including but not only: random forests, Bayesian Networks, Neural Networks, or other. 
...
```{r}
# Use a prediction error criteria (Adj^R2 BIC, Cp) to select the "best overall" model:
# names(reg.summary)

reg.summary$rsq
# Rsquare increases monotonically as more variables are included. 
# This is expected; the model containing all predictors will always have the smallest RSS and the largest R^2. 
...
The plot shows the size 10-11 is the minimum size for the subset for which the scores are within 0.2 standard deviations of optimum. We pick 5 as the parsimonious subset size.
...
for (j in 1:k) {
  gam.mgcv = mgcv::gam(ROAS ~ s(impr) + s(conv) + s(appConv) + s(CTR) + s(costPerCon) + s(CPM), data = trainTfo1[folds!=j,])  # interest returns an error 
...
We see the different R squares
...
in package gam, the analysist has to search for the effective df.
One can create a scope list with possible splines for each continous variable. 
Below, we considered splines up to maximum 5 degrees of freedom.
...
gam::step.Gam(gam.start, scope_list)
```
...
Not better than the previous gam model
...
```{r}
library(party)
cf1 <- cforest(trainTfo1$ROAS ~ . , data = trainTfo1[,-c(1, 3, 14:17)], control=cforest_unbiased(mtry=2,ntree=100))
